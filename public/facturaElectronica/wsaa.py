...
"M�dulo para obtener un ticket de autorizaci�n del web service WSAA de AFIP"
...
# Devuelve TA.xml (ticket de autorizaci�n de WSAA)
...
CERT = "certificado.crt"        # El certificado X.509 obtenido de Seg. Inf.
PRIVATEKEY = "privada.key"  # La clave privada del certificado CERT
PASSPHRASE = ""  # La contrase�a para firmar (si hay)
...
# Verificaci�n del web server remoto, necesario para verificar canal seguro
...
def sign_tra(tra,cert=CERT,privatekey=PRIVATEKEY,passphrase=""):
    ...
    if BIO:
        ...
        # soporte de contrase�a de encriptaci�n (clave privada, opcional)
...
def call_wsaa(cms, location = WSAAURL, proxy=None, trace=False, cacert=None):
    "Llamar web service con CMS para obtener ticket de autorizaci�n (TA)"
...
class WSAA(BaseWS):
    "Interfaz para el WebService de Autenticaci�n y Autorizaci�n"
    ...
    def AnalizarCertificado(self, crt, binary=False):
        "Carga un certificado digital y extrae los campos m�s importantes"
    ...
    def CrearPedidoCertificado(self, cuit="", empresa="", nombre="pyafipws",
        ...
        # normalizar encoding (reemplazar acentos, e�e, etc.)
    ...
    def LoginCMS(self, cms):
        "Obtener ticket de autorizaci�n (TA)"
    ...
    def CallWSAA(self, cms, url="", proxy=None):
        "Obtener ticket de autorizaci�n (TA) -version retrocompatible-"
    ...
    def Expirado(self, fecha=None):
        "Comprueba la fecha de expiraci�n, devuelve si ha expirado"
        if not fecha:
    ...
    def Autenticar(self, service, crt, key, wsdl=None, proxy=None, wrapper=None, cacert=None, cache=None, debug=False):
        "M�todo unificado para obtener el ticket de acceso (cacheado)"
        ...
        try:
            ...
            # creo el nombre para el archivo del TA (seg�n credenciales y ws) 
            ...
            if not os.path.exists(fn) or os.path.getsize(fn) == 0 or \
               os.path.getmtime(fn) + (DEFAULT_TTL) < time.time():    
                ...
                if not ok or self.Excepcion:
                    raise RuntimeError(u"Fallo la conexi�n: %s" % self.Excepcion)
                # llamar al m�todo remoto para solicitar el TA
...
if __name__=="__main__":
    ...
    if '--register' in sys.argv or '--unregister' in sys.argv:
        ...
        # instanciar el helper y revisar los par�metros
        wsaa = WSAA()
        ...
        # consultar el padr�n online de AFIP si no se especific� raz�n social:
        ...
        if not empresa:
            ...
            if ok and padron.denominacion:
                print u"Denominaci�n seg�n AFIP:", padron.denominacion
        ...
        print clave_privada
        print pedido_cert
        # convertir a terminaci�n de linea windows y abrir con bloc de notas
